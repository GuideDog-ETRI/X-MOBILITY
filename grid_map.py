...
def extract_gridmap(src_gmap, cntr_x, cntr_y, range_x, range_y, resol):
    """
    ...
    dst_gmap = GridMap(num_cols, num_rows, resol, cntr_x, cntr_y)
...
def extract_gridmap(src_map, nrows, ncols, resol, trans=None, rot=None):
    dst_map = GridMap(ncols, nrows, resol, 0, 0)
...
class GridMap:
    ...
    def __init__(self, width, height, resolution, center_x, center_y, init_val=0.0, free_val = 1.0):
        """__init__
    ...
    def get_all_free_positions(self):
        x_ids, y_ids = self.get_all_free_indices()
        x_pos, y_pos = self.calc_grid_center_xy_position_from_xy_index(x_ids, y_ids)
        return x_pos, y_pos

    def get_all_free_indices(self):
        x_ids, y_ids = self.get_all_indices()
        free = ~self.check_occupancy_from_xy_index(x_ids, y_ids)
    
        return x_ids[free], y_ids[free]
    
    def get_all_occupied_positions(self, dgrid=1):
        x_ids, y_ids = self.get_all_occupied_indices(dgrid=dgrid)
        x_pos, y_pos = self.calc_grid_center_xy_position_from_xy_index(x_ids, y_ids)
        return x_pos, y_pos

    def get_all_occupied_indices(self, dgrid=1):
        x_ids, y_ids = self.get_all_indices(dgrid=dgrid)
        occupied = self.check_occupancy_from_xy_index(x_ids, y_ids)
        return x_ids[occupied], y_ids[occupied]

    def get_all_positions(self, dgrid=1):
        xs, ys = np.mgrid[slice(self.min_x+self.resolution/2.0, self.max_x, dgrid*self.resolution),
                        slice(self.min_y+self.resolution/2.0, self.max_y, dgrid*self.resolution)]
        return xs.flatten(), ys.flatten()

    def get_all_indices(self, dgrid=1):
        x_ids, y_ids = np.mgrid[slice(0, self.width, dgrid),
                                slice(0, self.height, dgrid)]
    ...
    def set_value_from_xy_pos(self, x_pos, y_pos, val, bigger=False):
        """set_value_from_xy_pos
        ...
        x_ids, y_ids, valid = self.get_xy_index_from_xy_pos(x_pos, y_pos)
        x_ids, y_ids = x_ids[valid], y_ids[valid]
        self.set_value_from_xy_index(x_ids, y_ids, val, bigger)


    def set_value_from_xy_index(self, x_ids, y_ids, vals, bigger=False):
        ...
        :param val: grid value
        """
        # print(type(x_ids), type(x_ids)!=int)
        # if bigger and type(x_ids) != int:
        #     sorted_ids = np.flip(np.argsort(vals))
        #     x_ids, y_ids, vals = x_ids[sorted_ids], y_ids[sorted_ids], vals[sorted_ids]
        #     unique_xy_ids, unique_ids = np.unique(np.stack((x_ids, y_ids), axis=1), axis=0, return_index=True)
        #     x_ids, y_ids, vals = unique_xy_ids[:, 0], unique_xy_ids[:, 1], vals[unique_ids]
    ...
    def check_occupancy_from_xy_pos(self, x_pos, y_pos):
        ...
        return vals > self.free_val # self.init_val

    def check_occupancy_from_xy_index(self, x_ids, y_ids):
        vals = self.get_value_from_xy_index(x_ids, y_ids)
        return vals > self.free_val

    # def set_configuration_space(self, robot_radius=1.0):
    #     oc_x_pos, oc_y_pos = self.get_all_occupied_positions()
    #     # fr_x_pos, fr_y_pos = self.get_all_free_positions()
    #     # fr_x_ids, fr_y_ids = self.get_all_free_indices()
    #     oc_pos = np.stack((oc_x_pos, oc_y_pos), axis=1)
    #     # fr_pos = np.stack((fr_x_pos, fr_y_pos), axis=1)

    #     cdist = distance.cdist(oc_pos, fr_pos)
    #     ids = cdist < robot_radius
    #     print(ids.shape)
        
    #     # for idx, (ox, oy) in enumerate(zip(oc_x_pos, oc_y_pos)):
    #     #     dist = np.hypot(fr_x_pos - ox, fr_y_pos - oy)
    #     #     cspace = dist < robot_radius
    #     #     self.set_value_from_xy_index(fr_x_ids[cspace], fr_y_ids[cspace], 1)
    #     #     fr_x_pos = fr_x_pos[~cspace]
    #     #     fr_y_pos = fr_y_pos[~cspace]
    #     #     fr_x_ids = fr_x_ids[~cspace]
    #     #     fr_y_ids = fr_y_ids[~cspace]

    def set_value_from_polygon(self, pol_x, pol_y, val, inside=True, free_only=False):
        ...
        if free_only:
            for x_idx, y_idx, x, y in zip(x_ids, y_ids, x_pos, y_pos):
                ...
                if self.grid_map[y_idx, x_idx] == self.init_val and flag is inside:
